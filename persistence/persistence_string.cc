...
public:
  ...
  PersistentString(std::string path):
                  data_path_(path) {}
...
void PersistentString::Init() {
  int64_t fd = open(data_path_.c_str(),
                    O_RDWR | O_CREAT, kFileMode);
  if (fd < 0) {
...
int PersistentString::DeCoding(std::string *data) {
  char buf[4096];
  int n = pread(persistent_fd_, buf, 
                sizeof(buf), 0);
  if (n <= 0) {
