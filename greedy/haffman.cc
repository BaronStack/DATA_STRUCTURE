...
void initNode(priority_queue<Node> &q, map<char, int> buf) {
  for (map<char, int>::iterator it = buf.begin(); it != buf.end(); it++) {
...
void huffmanEncode(Node *root, string &prefix, map<char, string> &result) {
...
bool huffmanDecode(string des, map<char, string> res, string &result) {
  if (des == "") {
    return false;
  }
